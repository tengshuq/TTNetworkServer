...
/**
 手动设置客户端cookie(服务端获取的cookie在[TTNetworkConfig cookieEnabled]设置后自动启用)
...
/**
 ...
 @param failure    失败的回调
 @return           返回当前请求的Task
 @discussion       请求成功的数据会转为字典（包括XML数据），如果返回的数据无法转换，则返回@{@"result":reponse}，response为请求返回的数据，id类型
 */
...
/**
 ...
 @param failure     失败的回调
 @return            返回当前请求的Task
 @discussion        请求成功的数据会转为字典（包括XML数据），如果返回的数据无法转换，则返回@{@"result":reponse}的字典，response为请求返回的数据，id类型
 */
...
/**
 ...
 @param parameters  上传参数
 @param name        文件对应服务器的name
 @param size        需要压缩上传的图片大小（等比例压缩原图后得出的大小，单位为M),nil或者<= 0,则表示不压缩
 @param images      图片数组
...
/**
 ...
 @param success 请求成功的回调，如果其中某一个请求发生错误，回调的结果对应位置为<null>
 @param failure 请求错误的回调，如果对应的位置没有error，则回调的结果为<null>
 @param task    task内容的回调，回调顺序对应加入的请求的顺序
 */
+ (void)startBatchRequest:(NSArray<TTNetworkServer *> *)request
                  success:(void(^)(NSArray<id> *responses))success
                  failure:(void(^)(NSArray<id> *errors))failure
                     task:(void(^)(NSArray<NSURLSessionDataTask *> *tasks))task;
